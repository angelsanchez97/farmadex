...
def test_resultado_ordenado_por_tiempo(entorno):
    ...
    assert datos["misiones"][0]["minutos"] == 146.8
    ...
    assert pestana.calcular()["misiones"][0]["minutos"] == 505.0
...
def test_ficha_por_donde_empezar(entorno):
    ...
    assert "Captura, Earth" in pieza and "(+1)" in pieza and "~2.4 h" in pieza
...
def test_la_ficha_del_buscador_usa_la_ruta_completa(entorno):
    ...
    assert "hasta tener la pieza" in html and "Lith T1" in html and "~2.4 h" in html
